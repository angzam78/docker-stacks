...
docker run --rm -it --volume $(pwd):/certs cockroachdb/cockroach cert create-node localhost 172.17.0.1 crdb1 crdb2 crdb3 --certs-dir=/certs/certs --ca-key=/certs/ca.key
